...
public class LinkedFileCacheDuplicateSupport {
	...
	/**
	 * active the support for internal ORP logging data format
	 */
	private boolean activeInternalORPLogFormat = false;
	 ...
	 * @return success
	 */
	public boolean initialize(final String _groundTruthFilename, final long _desiredEvaluationTimespan, final boolean activeInternalORPLogFormat) throws IOException {
		this.activeInternalORPLogFormat = activeInternalORPLogFormat;
		return initialize(_groundTruthFilename, _desiredEvaluationTimespan);
	}
	/**
	 * Initialize. Read the first line into the buffer
	 * 
		...
		if (inLogFile.isFile()) {
			...
			File[] childs = inLogFile.listFiles(new FilenameFilter() {
				...
				public boolean accept(File dir, String name) {
					...
					return fileName.endsWith(".gz") || fileName.endsWith("data.idomaar.txt");
				}
			});
	...
	public boolean adaptBufferForNewTimeStamp(final long timeStamp) throws Exception{
		...
		while (newestTimeStamp < requestedNewestTimeStamp && moreLinesToRead) {
			...
			Impression impression = this.activeInternalORPLogFormat
				? Impression.createImpressionFromORPLogLine(line)
				: Impression.createImpressionFrom4CollCSV(line);
